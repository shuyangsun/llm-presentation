...
- `src/asr/media.py` extracts the first audio stream as mono 16 kHz PCM WAV through `ffmpeg` for backends that require normalized audio input.
- `src/asr/transcribe_vtt.py` selects the backend, groups words by cue length, duration, punctuation, and pauses, then writes a `.vtt` file.
